...
output "mcp_server_fqdn" {
  description = "FQDN of the private MCP container app. Resolves to a private IP on snet-mcp via the Container Apps Environment's auto-provisioned private DNS zone."
  value       = try(trimprefix(module.mcp_container_app.fqdn_url, "https://"), "")
}

output "mcp_server_url" {
  description = "URL of the private MCP server's no-auth Streamable HTTP endpoint, used by the hosted agent."
  value       = try("${module.mcp_container_app.fqdn_url}/noauth/mcp", "")
}

output "mcp_uami_client_id" {
  description = "Client ID of the user-assigned managed identity that the MCP container app uses to pull from ACR."
