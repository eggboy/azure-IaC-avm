...
PROJECT_ENDPOINT = os.environ.get(
    "PROJECT_ENDPOINT", "https://ais-mar16-dev-e41t.services.ai.azure.com/api/projects/proj-project-e41t"
)
...
MCP_SERVER_PUBLIC = os.environ.get(
    "MCP_SERVER_PUBLIC", "https://my-mcp-server.icyplant-f44af130.eastus2.azurecontainerapps.io/noauth/mcp"
)

# Private MCP server (internal, only accessible from VNet via Data Proxy)
MCP_SERVER_PRIVATE = os.environ.get(
    "MCP_SERVER_PRIVATE", "https://my-mcp-server.icyplant-f44af130.eastus2.azurecontainerapps.io/noauth/mcp"
)
        ...
        call_data = json.dumps(
            {
                ...
                "id": 2,
                "method": "tools/call",
                "params": {"name": "add", "arguments": {"a": 2, "b": 4}},
            }
        ).encode("utf-8")
...
def main():
    ...
    parser.add_argument("--retry", type=int, default=1, help="Number of retries for agent tests (default: 1)")
    ...
    results["connectivity"] = test_mcp_connectivity(
        mcp_url, "Public MCP Server" if mcp_url == MCP_SERVER_PUBLIC else "Private MCP Server"
    )
